    ...
    extends generated_media_file_list
{
    ...
    public function load_picture()
    {
     ...
     require_once(__DB_ROOT__.'/class.image_list.php');
     ...
     media_file.picture_id > 0 )";
     $this->basic_load( $prepare_statement );
     
     $image_list = new image_list();
     for( $n = 0; $n < $this->get_item_count(); $n++ )
     {
     $media_file = $this->get_item( $n );
     
     $new_image = new image();
     $new_image->set_id( $media_file->get_picture_id() ); 
     $new_image->load();
     
     $image_list->add_item( $new_image );
     }
     return $image_list;
    }
    /**
