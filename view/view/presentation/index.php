...
$_TRANSLATIONS["el"] = array(
	...
	"size" => "???e???",
	"last_changed" => "???p?p???µ???",
	"total_used_space" => "???s?µ?p???µ???? ?????",
	"free_space" => "??e??e??? ?????",
	"password" => "??s??eta? ??d???",
	"upload" => "F??t?s?",
	"failed_upload" => "?p?t???a f??t?s?? a??e???!",
	"failed_move" => "?p?t???a µetaf???? a??e??? st?? ?at?????? f??e??!",
	"wrong_password" => "????? ??d????",
	"make_directory" => "??µ??????a ???? fa?????",
	"new_dir_failed" => "?p?t???a d?µ??????a? ???? fa?????",
	"chmod_dir_failed" => "?p?t???a t??p?p???s?? d??a??µ?t?? fa?????",
	"unable_to_read_dir" => "Unable to read directory",
	"location" => "Location",
	"root" => "Root"
);
