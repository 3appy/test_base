    ...
    extends service_operation
{
    ...
    public function exist_team( $top_team, $team_name )
    {
     $exist = FALSE;
     
     // exist this new team
     $sub_team_list = $top_team->get_subteam_list();
     $count = $sub_team_list->get_item_count();
     for ( $n=0; (($n<$count) AND ($exist == FALSE)); $n++ )
     {
     $team = $sub_team_list->get_item( $n );
    ...
    public function generate_new_team()
    {
     ...
     if( !( $this->exist_team( $top_team, $this->team_name )))
     {
     $new_team = new team();
