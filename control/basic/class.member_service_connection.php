    ...
    extends basic_service_connection
{
    ...
    public function set_member_follower_connection_csv($follower_id)
    {
     if( defined('__ROOT_DATA__') == FALSE )
     { define('__ROOT_DATA__', $this->get_root_data() ); }
     require_once(__ROOT_DATA__.'class.member_follower.php');
     ...
     $connection = $con->find_connection( $follower_id, $this->member_id );
     if( $connection == NULL )
     {
     // there is no relation between parent part and child
     $con->set_follower_id( $follower_id );
     $con->set_member_id( $this->member_id );
     ...
     {
     if( $connection->get_status() == (int)5 ) 
     {
     // There is already a relation between those two (5 - accepted)
     $success = FALSE;    
     }
     else
     {
     $connection->set_status( (int)5 );
     $connection->update();
     $success = TRUE;    
     }
     }
