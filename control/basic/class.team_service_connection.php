    ...
    extends basic_service_connection
{
    ...
    public function set_team_admin_connection_csv()
    {
     if( defined('__ROOT_DATA__') == FALSE )
     { define('__ROOT_DATA__', $this->get_root_data() ); }
     require_once(__ROOT_DATA__.'class.team_member.php');
     ...
     $con = new team_member();
     // ist es schon ein Admin ?
     $id = $con->find_member( $team_id, $member_id, (int)2  );
     if ( $id > 0 )
     { $success = FALSE; }
     else
     {
     $id = $con->find_member_statusless($team_id, $member_id);
     if ( $id > 0 )
     {
     // ist er Teil der Gruppe, aber kein Admin ? 
     $con->set_id( $id );
     $con->load();
    ...
    public function set_team_member_connection_csv()
    {
     if( defined('__ROOT_DATA__') == FALSE )
     { define('__ROOT_DATA__', $this->get_root_data() ); }
     require_once(__ROOT_DATA__.'class.team_member.php');
     ...
     $con = new team_member();
     // ist es schon ein Admin ?
     $id = $con->find_member( $team_id, $member_id, (int)5  );
     if ( $id > 0 )
     { $success = FALSE; }
     else
     {
     ...
     if ( $id > 0 )
     {
     // ist er Teil der Gruppe, aber kein Member ?
     $con->set_id( $id );
     $con->load();
     $con->set_status( (int)5 );
     $con->update();
